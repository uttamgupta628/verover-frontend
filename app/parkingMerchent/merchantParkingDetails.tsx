...
import { ArrowLeft, Trash2, ChevronLeft, ChevronRight, X, AlertCircle, Plus } from 'lucide-react-native';
...
const ParkingDetails = () => {
    ...
    const parkingLotData = parkingLotDataString ? JSON.parse(parkingLotDataString) : undefined;

    const fetchParkingLotDetails = useCallback(async (showLoader = true) => {
        if (!parkingLotId) {
        ...
        try {
            ...
            // Since axiosInstance already includes "/api" in baseURL
            const response = await axiosInstance.get(`/merchants/parkinglot/${parkingLotId}`, {
                headers: {
            ...
            if (response.data && response.data.success && response.data.data) {
                ...
                const formattedSpacesList = fetchedData.spacesList ?
                    Object.fromEntries(Object.entries(fetchedData.spacesList).map(([key, value]: [string, any]) => [key, { count: value.count, price: value.price }])) : {};
                ...
                const fullParkingLotData: IParkingLot = {
                    ...
                    gpsLocation: fetchedData.gpsLocation || { type: 'Point', coordinates: [0, 0] },
                    price: fetchedData.price || 0,
                };

                setParkingLotDetails(fullParkingLotData);
                setFormData(fullParkingLotData);
                setLocalImages(fullParkingLotData.images.map(uri => ({
                    uri,
                    name: uri.split('/').pop() || 'image.jpg',
                    type: 'image/jpeg',
                })));
            } else {
                throw new Error(response.data?.message || 'Invalid response structure or parking lot not found.');
            }
        } catch (err: any) {
            console.error('Error fetching parking lot details:', err.response?.data || err.message);
            setError('Failed to load parking lot details: ' + (err.response?.data?.message || err.message || 'An unexpected error occurred.'));
        } finally {
  ...
  Alert.alert(
      ...
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            Alert.alert('Success', 'Parking Lot has been deleted successfully.');
        ...
        useCallback(() => {
            if (parkingLotId) {
                ...
                const formattedSpacesList = parkingLotData.spacesList ?
                    Object.fromEntries(Object.entries(parkingLotData.spacesList).map(([key, value]: [string, any]) => [key, { count: value.count, price: value.price }])) : {};
                ...
                setParkingLotDetails(fullParkingLotData);
                setFormData(fullParkingLotData);
                setLocalImages(fullParkingLotData.images.map(uri => ({ uri, name: uri.split('/').pop() || 'image.jpg', type: 'image/jpeg' })));
            } else {
                setError('No parking lot information provided.');
            }
            setIsEditing(false);
        }, [fetchParkingLotDetails, parkingLotId, parkingLotData])
    );
    ...
    const handleUpdateParkingLot = async () => {
        ...
        try {
            ...
            data.append('gpsLocation', JSON.stringify(formData.gpsLocation || { type: 'Point', coordinates: [0, 0] }));

            // Handle images
            const newLocalFiles = localImages.filter(img => !img.uri.startsWith('http'));
            const existingImages = localImages.filter(img => img.uri.startsWith('http')).map(img => img.uri);
            ...
            console.error('Error updating parking lot:', err.response?.data || err.message);
    ...
    const handleCancelEdit = () => {
        ...
        setLocalImages(parkingLotDetails?.images.map(uri => ({
            uri,
            name: uri.split('/').pop() || 'image.jpg',
            type: 'image/jpeg',
        })) || []);
        ...
        return (
            <View style={styles.container}>
                ...
                <View style={styles.noDataContainer}>
                    <Text style={styles.noDataText}>No parking lot information available</Text>
                    <TouchableOpacity
    ...
    return (
        ...
        >
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle} numberOfLines={1}>{parkingLotDetails.parkingName}</Text>
            ...
            <View style={styles.card}>
                <View style={styles.infoContent}>
                    ...
                    {isEditing ? (
                        ...
                        <Text style={styles.priceValue}>${parkingLotDetails.price?.toFixed(2) || '0.00'}/hr</Text>
                    )}
            ...
            <View style={styles.card}>
                ...
                {isEditing ? (
                    ...
                    <Text style={styles.aboutText}>{parkingLotDetails.about || 'No description provided'}</Text>
                )}
            </View>
            ...
            <View style={styles.card}>
                ...
                {isEditing ? (
                    ...
                    <Text style={styles.displayValue}>{parkingLotDetails.contactNumber}</Text>
                )}
                ...
                {isEditing ? (
                    ...
                    <Text style={styles.displayValue}>{parkingLotDetails.email || 'Not provided'}</Text>
                )}
            </View>
            ...
            <View style={styles.card}>
                ...
                {Object.entries(formData.spacesList || parkingLotDetails.spacesList || {}).map(([zone, spaceInfo]) => (
                        ...
                        {isEditing ? (
                            ...
                            <View style={styles.spaceInfoRow}>
                                ...
                                <View style={styles.spaceInfoItem}>
                                    ...
                                    <Text style={styles.spaceInfoValue}>${spaceInfo?.price?.toFixed(2) || '0.00'}</Text>
                                </View>
                            </View>
                        )}
                    </View>
            ...
            {(!formData.is24x7 && !parkingLotDetails.is24x7) && (
                <View style={styles.card}>
                    ...
                    {(formData.generalAvailable || parkingLotDetails.generalAvailable)?.map((day, index) => (
                            ...
                            {day.isOpen && !day.is24Hours && (
                                <>
                                    ...
                                    {isEditing ? (
                                        ...
                                        >
                                            <Text style={styles.timeInputText}>{day.openTime || 'Select open time'}</Text>
                                        </TouchableOpacity>
                                    ) : (
                                        <Text style={styles.displayValue}>{day.openTime || 'Not set'}</Text>
                                    )}
                                    ...
                                    {isEditing ? (
                                        ...
                                        >
                                            <Text style={styles.timeInputText}>{day.closeTime || 'Select close time'}</Text>
                                        </TouchableOpacity>
                                    ) : (
                                        <Text style={styles.displayValue}>{day.closeTime || 'Not set'}</Text>
                                    )}
                                </>
                            )}
            ...
            <View style={styles.card}>
                ...
                <Text style={styles.displayValue}>
                    {parkingLotDetails.gpsLocation?.coordinates[1]?.toFixed(6)}, {parkingLotDetails.gpsLocation?.coordinates[0]?.toFixed(6)}
                </Text>
            </View>
...
const styles = StyleSheet.create({
    ...
    header: {
        ...
        marginTop: Platform.OS === 'ios' ? responsiveHeight(6) : responsiveHeight(0),
    },
    headerTitle: {
