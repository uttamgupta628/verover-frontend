...
interface ParkingLotFormData {
  ...
  is24x7: boolean;
  gpsLocation: { type: string; coordinates: number[] };
...
const RegisterParkingLot = () => {
  ...
  useEffect(() => {
    if (parkingLotId) fetchParkingLotDetails();
    requestLocationPermissionAndGetLocation();
  }, [parkingLotId]);

  useEffect(() => {
    const total = Object.values(formData.spacesList).reduce(
      (acc, zone) => acc + (zone.count || 0),
      0,
    );
  ...
  const fetchParkingLotDetails = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        totalSlot: d.totalSlot || 0,
        spacesList: Object.fromEntries(d.spacesList),
        generalAvailable: d.generalAvailable,
        is24x7: d.is24x7,
  ...
  const addZone = () => {
    ...
    const newZone =
      zones.length > 0
        ? String.fromCharCode(zones[zones.length - 1].charCodeAt(0) + 1)
        : "A";
    setFormData((prev) => ({
      ...prev,
  ...
  const handleSubmit = async () => {
    ...
    if (formData.monthlyChargeEnabled && formData.monthlyRate <= 0) {
      return Alert.alert(
        "Validation Error",
        "Please enter a valid monthly rate greater than 0.",
      );
    }
    ...
    try {
      ...
      data.append(
        "spacesList",
        JSON.stringify(Object.fromEntries(Object.entries(formData.spacesList))),
      );
      data.append(
        "generalAvailable",
      ...
      data.append(
        ...
        formData.monthlyChargeEnabled.toString(),
      );
      data.append("monthlyRate", formData.monthlyRate.toString());
      // Note: dailyRateEnabled / dailyRates are saved via a separate
      // PATCH /merchants/daily-rate-settings call after registration.
      ...
      const endpoint = parkingLotId
        ...
        : "/merchants/parkinglot/registration";
      ...
      // ── Step 2: save daily rate using the lot _id returned by registration ──
      // For new lots  → response.data.data.parkingLot._id
      // For edits     → parkingLotId already known
      const savedLotId =
        parkingLotId ??
        response.data?.data?.parkingLot?._id ??
        response.data?.parkingLot?._id ??
        response.data?.data?._id;

      // Debug log — remove after confirming correct _id extraction
      console.log("registration response body:", JSON.stringify(response.data));
      console.log("savedLotId:", savedLotId);

      if (savedLotId && formData.dailyRateEnabled) {
        try {
          await axiosInstance.patch(
            "/merchants/daily-rate-settings",
            {
              ...
              dailyRates:       formData.dailyRates.map(({ label, fromTime, toTime, price }) => ({
                label,
                fromTime,
                toTime,
                price,
          ...
          // Parking lot was saved — warn but don't block navigation
          Alert.alert(
            "Warning",
            "Parking lot saved, but daily rate slots could not be saved: " +
              (dailyRateErr.response?.data?.message || dailyRateErr.message),
          );
          router.back();
          return;
        }
      } else if (savedLotId && !formData.dailyRateEnabled) {
        // If editing and daily rate was disabled, clear existing slots
        if (parkingLotId) {
          try {
            await axiosInstance.patch(
              "/merchants/daily-rate-settings",
      ...
      Alert.alert(
        "Success",
        `Parking Lot ${parkingLotId ? "updated" : "created"} successfully`,
      );
      router.back();
    } catch (error: any) {
      Alert.alert(
      ...
      >
        ...
        <View style={styles.card}>
          <Text style={styles.sectionTitle}>
            Zones & Spaces (Total: {formData.totalSlot})
          </Text>
        ...
        <View style={styles.card}>
          ...
          <TextInput
            style={[
              styles.input,
              { height: responsiveHeight(10), textAlignVertical: "top" },
            ]}
            ...
            <View style={styles.monthlyHeaderLeft}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.monthlySubtitle}>
                  Allow customers to subscribe on a monthly basis
                </Text>
              </View>
            </View>
            <Switch
            ...
            <View style={styles.monthlyRateContainer}>
              ...
              <View style={styles.rateInputRow}>
                ...
                <TextInput
                  ...
                  onChangeText={(text) =>
                    handleChange("monthlyRate", parseFloat(text) || 0)
                  }
                  placeholder="0.00"
                  keyboardType="decimal-pad"
          ...
          {formData.dailyRateEnabled && (
            <View style={styles.dailyRateBody}>
              <Text style={styles.dailyRateHint}>
                Each window is charged once the booking enters it, regardless of
                duration. The last slot repeats for any overflow beyond midnight.
              </Text>
...
const styles = StyleSheet.create({
  ...
  vehicleTypeButtonTextActive: { color: colors.brandColor },
  zoneInputContainer: {
    flexDirection: "row",
    alignItems: "center",
  ...
  monthlyHeaderLeft: { flexDirection: "row", alignItems: "center", flex: 1 },
  planIconWrap: {
    width: 36,
    height: 36,
    borderRadius: 10,
  ...
  monthlyRateContainer: { marginTop: responsiveHeight(0.5) },
  rateInputRow: {
    flexDirection: "row",
    alignItems: "center",
    borderWidth: 1.5,
