...
const MerchantResidenceForm = () => {
    ...
    const fetchResidenceDetails = async () => {
        setIsLoading(true);
        try {
            const response = await axiosInstance.get(`/api/merchants/residence/${residenceId}`);
            ...
            if (data.images && data.images.length > 0) {
                setImages(data.images.map((uri: string) => ({
                    uri,
    ...
    const handleSubmit = async () => {
        if (!formData.residenceName || !formData.address || !formData.contactNumber || !formData.about || formData.price <= 0) {
        ...
        try {
            ...
            Object.keys(formData).forEach(key => {
                ...
                if (['generalAvailable', 'gpsLocation', 'emergencyContact'].includes(formKey)) {
                    ...
                    const arrayValue = (value as string).split(',').map(s => s.trim()).filter(Boolean);
            ...
            Alert.alert('Success', `Residence ${residenceId ? 'updated' : 'created'} successfully`);
            router.back();
        } catch (error: any) {
            console.error('Error submitting residence:', error.response?.data || error.message);
            Alert.alert('Error', error.response?.data?.message || 'Failed to submit residence details.');
        } finally {
            setIsLoading(false);
        }
    };
