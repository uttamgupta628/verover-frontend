...
const MerchantGarageList = () => {
    ...
    const fetchGarages = useCallback(async () => {
        try {
            ...
            console.log('[Garage] Full response:', JSON.stringify(response.data, null, 2));

            // Backend returns: ApiResponse(200, result) where result is a flat array
            // ApiResponse structure is typically: { statusCode, data, message }
            // So response.data = { statusCode: 200, data: [...garages] }
            // OR response.data = [...garages] directly — handle both
            let rawList: any[] = [];

            if (Array.isArray(response.data)) {
                // Case 1: raw array returned directly
                rawList = response.data;
            } else if (Array.isArray(response.data?.data)) {
                // Case 2: wrapped in { data: [...] }
                rawList = response.data.data;
            } else if (response.data?.data && typeof response.data.data === 'object') {
                // Case 3: single object returned (edge case)
                rawList = [response.data.data];
            }

            console.log(`[Garage] Parsed ${rawList.length} items from response`);

            const formattedGarages = rawList.map((garage: any) => ({
                ...garage,
    ...
    const handleRefresh = () => fetchGarages();
    ...
    const calculateTotalSlots = (spacesList: Garage['spacesList']) => {
        if (!spacesList) return 0;
        return Object.values(spacesList).reduce((total, zone) => {
            return total + (typeof zone?.count === 'number' ? zone.count : 0);
        }, 0);
    };
        ...
        <SafeAreaView style={styles.container}>
            ...
            {garages.length === 0 ? (
                <View style={styles.emptyContainer}>
                    ...
                    <TouchableOpacity style={styles.addButton} onPress={handleAddGarage}>
...
const styles = StyleSheet.create({
    ...
    headerTitle: { fontSize: responsiveFontSize(2.2), fontWeight: 'bold', color: colors.black },
    ...
    imageContainer: { height: responsiveHeight(20), width: '100%', backgroundColor: colors.lightGray },
    garageImage: { height: '100%', width: '100%' },
    infoContainer: { padding: responsiveWidth(4) },
    garageName: { fontSize: responsiveFontSize(2), fontWeight: 'bold', color: colors.black, marginBottom: responsiveHeight(0.5) },
    address: { fontSize: responsiveFontSize(1.6), color: colors.gray, marginBottom: responsiveHeight(1), lineHeight: responsiveHeight(2) },
    detailsRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: responsiveHeight(1) },
    priceContainer: { flexDirection: 'row', alignItems: 'center' },
    priceLabel: { fontSize: responsiveFontSize(1.6), color: colors.gray, marginRight: responsiveWidth(1) },
    priceValue: { fontSize: responsiveFontSize(1.8), fontWeight: 'bold', color: colors.brandColor },
    slotsContainer: { flexDirection: 'row', alignItems: 'center' },
    slotsLabel: { fontSize: responsiveFontSize(1.6), color: colors.gray, marginRight: responsiveWidth(1) },
    slotsValue: { fontSize: responsiveFontSize(1.8), fontWeight: 'bold', color: colors.black },
    footerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: responsiveHeight(1) },
    contactContainer: { flexDirection: 'row', alignItems: 'center', flex: 1, marginRight: responsiveWidth(2) },
    contactText: { fontSize: responsiveFontSize(1.6), color: colors.gray, marginLeft: responsiveWidth(1) },
    ...
    closedBadge: { backgroundColor: '#F44336' },
    statusText: { fontSize: responsiveFontSize(1.4), fontWeight: 'bold', color: colors.white },
    emptyContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: responsiveWidth(10) },
    emptyImage: { width: responsiveWidth(60), height: responsiveWidth(60), marginBottom: responsiveHeight(3) },
    emptyText: { fontSize: responsiveFontSize(2), color: colors.gray, textAlign: 'center', marginBottom: responsiveHeight(2) },
    addButton: { backgroundColor: colors.brandColor, paddingVertical: responsiveHeight(1.5), paddingHorizontal: responsiveWidth(8), borderRadius: 8 },
    addButtonText: { color: '#FFF', fontSize: responsiveFontSize(1.8), fontWeight: 'bold' },
